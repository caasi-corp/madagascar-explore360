...
export const seedTours = async (db: IDBPDatabase<NorthGascarDB>): Promise<void> => {
  console.log("Vérification des circuits touristiques...");
  
  try {
    // Vérifier si des circuits existent déjà
    const existingTours = await db.count('tours');
    if (existingTours > 0) {
      console.log(`${existingTours} circuits existent déjà dans la base de données`);
      return;
    }
